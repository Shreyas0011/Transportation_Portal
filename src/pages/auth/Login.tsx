...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data = await transportApi.login({ email });
  ...
  const handleQuickLogin = (roleEmail: string) => {
    setEmail(roleEmail);
    setLoading(true);
    transportApi.login({ email: roleEmail })
      .then((data) => {
        toast.success(`Welcome back, ${data.user.name}!`);
        onLoginSuccess(data.user, data.accessToken);
      })
        ...
        <div style={{ marginTop: '28px', borderTop: '1px solid #e2e8f0', paddingTop: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            ...
            <button
              className="modern-btn-secondary"
              style={{ fontSize: '11px', padding: '8px 10px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}
              onClick={() => handleQuickLogin('parent1@transcend.org')}
              disabled={loading}
            >
              <span style={{ fontWeight: 700, color: '#7c3aed' }}>Parent (Aarav)</span>
              <span style={{ fontSize: '9px', fontWeight: 500, color: '#94a3b8' }}>parent1@transcend.org</span>
            </button>
            <button
              className="modern-btn-secondary"
              style={{ fontSize: '11px', padding: '8px 10px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}
              onClick={() => handleQuickLogin('driver1@transcend.org')}
              disabled={loading}
            >
              <span style={{ fontWeight: 700, color: '#10b981' }}>Driver (Manjunath)</span>
              <span style={{ fontSize: '9px', fontWeight: 500, color: '#94a3b8' }}>driver1@transcend.org</span>
            </button>
            <button
              className="modern-btn-secondary"
              style={{ fontSize: '11px', padding: '8px 10px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}
